...
fun calculateOggDuration(oggFileLength: Long, oggFileInputStream: InputStream): Double {
    var rate = -1
    var length = -1

    val size = oggFileLength.toInt()
    val t = ByteArray(size)

    oggFileInputStream.use { stream ->
        stream.read(t)
